...
public class BoardExample {
	...
	public static void main(String[] args) {
		...
		// String 형태라서 null로 초기화됨
		// notice 변수가 참조하고 있는 배열의 크기는 100으로 설정, 현 배열 index가 참조하고 있는 배열의 크기는 4로 설정
		...
		while (run) {// run이 false 될 때 까지 수행
			...
			for (int a = 100; a > 0; a--) {
				String strA = String.valueOf(a);// int형인 a를 valueOf를 통해 String형태로 변환
				for (String[] n : notice) { // 2차원 배열인 notice에서 한 행씩 가져와 문자열 배열 n에 저장하는 반복문
			...
			if (menu == 1) {
				...
				// 게시판에 글이 없으면 보기 선택 x VS 번호 선택했을떄 null 일때만 오류 값 잡기(선택)

				System.out.print("게시물 번호 : ");
				String num = scanner.nextLine();
				boolean flag = false;// 게시물 번호가 notice 배열 안에 있는지 확인하는 변수 [true면 있고 false면 없음]
				for (String[] n : notice) {
				...
				for (int i = 0; i < 100; i++) {
					if (notice[i][0] != null && notice[i][0].equals(num)) {
						...
						if (!newTitle.replace(" ", "").equals("")) {
							// !논리 연산자를 사용하여서 공백이 아닌 경우에만 수정을 진행
							notice[i][1] = newTitle;
						}
